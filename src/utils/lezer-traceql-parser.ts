 ...
 *
 * Complex Combinations (within single spanset):
 ...
 *
 * - Multiple spansets: {service.name="A"} && {status=error}
 *   Reason: Traces-drilldown only supports a single spanset. Only the first spanset will be extracted.
 *
...
export interface TraceQLParseError {
  type:
    ...
    | 'unsupported_structural_operator'
    | 'unsupported_pipeline'
    | 'unsupported_union_operator';
  message: string;
  query: string;
}
...
export function parseTraceQLQuery(query: string): TraceQLParseResult | null {
  ...
  try {
    ...
    // Traces-drilldown only supports one spanset, so extract filters from the first spanset only
    ...
    // Report if there are multiple spansets that were ignored
    if (allSpansetFilters.length > 1) {
      errors.push({
        type: 'unsupported_or_between_fields',
        message: `Query contains ${allSpansetFilters.length} spansets. Only the first spanset was extracted because traces-drilldown supports a single spanset only.`,
        query: cleanQuery,
      });
    }
