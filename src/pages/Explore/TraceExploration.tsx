...
import {
  TraceQLIssueDetector,
  TraceQLConfigWarning,
} from "../../components/Explore/TraceQLIssueDetector";
...
export interface TraceExplorationState
  extends SharedExplorationState, SceneObjectState {
...
export class TraceExploration extends SceneObjectBase<TraceExplorationState> {
  ...
  public constructor(state: Partial<TraceExplorationState>) {
    ...
    super({
      ...
      $variables:
        state.$variables ?? getVariableSet(state as TraceExplorationState),
  ...
  public _onActivate() {
    ...
    const datasourceVar = sceneGraph.lookupVariable(
      VAR_DATASOURCE,
      this,
  ...
  updateFromUrl(values: SceneObjectUrlValues) {
    ...
    if (values.traceId || values.spanId) {
      stateUpdate.traceId = values.traceId
        ? (values.traceId as string)
        : undefined;
      stateUpdate.spanId = values.spanId
        ? (values.spanId as string)
        : undefined;
    }
...
export class TraceExplorationScene extends SceneObjectBase {
  static Component = ({
    model,
  }: SceneComponentProps<TraceExplorationScene>) => {
    ...
    return (
      <div className={styles.container} id="trace-exploration">
        {hasIssue && issueDetector && (
          <TraceQLConfigWarning detector={issueDetector} />
        )}
        {embedded ? (
          <EmbeddedHeader model={model} />
        ) : (
          <TraceExplorationHeader controls={controls} model={model} />
        )}
        ...
        <DataLinksCustomContext
          embedded={embedded}
          timeRange={timeRangeState?.value}
        >
          <SmartDrawer
            isOpen={!!drawerScene && !!traceId}
            onClose={onClose}
            title={t(
              "trace-exploration.drawer.view-trace",
              "View trace {{traceId}}",
              { traceId },
            )}
            embedded={embedded}
...
export const useServiceName = (model: SceneObject) => {
  ...
  const getServiceNameFromFilters = (filters: AdHocVariableFilter[]) => {
    const serviceNameFilter = filters.find(
      (f) => f.key === "resource.service.name",
    );
    return serviceNameFilter?.operator === "=" ||
      serviceNameFilter?.operator === "=~"
...
const EmbeddedHeader = ({
  ...
  const { returnToPreviousSource, embeddedMini, hideRedPanels } =
    explorationState;
  const styles = useStyles2(getStyles, true, embeddedMini);
  const filtersVariable = getFiltersVariable(traceExploration);
  const primarySignalVariable = getPrimarySignalVariable(traceExploration);
  const timeRangeControl = traceExploration.state.controls.find(
    (control) => control instanceof SceneTimePicker,
  );
  const refreshControl = traceExploration.state.controls.find(
    (control) => control instanceof SceneRefreshPicker,
  );
  ...
  const [explorationUrl, setExplorationUrl] = React.useState(() =>
    getUrlForExploration(traceExploration),
  );
          ...
          <LinkButton
            ...
            onClick={() => {
              ...
              reportAppInteraction(
                ...
                USER_EVENTS_ACTIONS.common.go_to_full_app_clicked,
              );
            }}
          >
            <Trans i18nKey="trace-exploration.embedded-header.traces-drilldown">
              Traces Drilldown
            </Trans>
          </LinkButton>
          {timeRangeControl && !hideRedPanels && (
            <timeRangeControl.Component model={timeRangeControl} />
          )}
          {refreshControl && !hideRedPanels && (
            <refreshControl.Component model={refreshControl} />
          )}
        </Stack>
      </Stack>
    </div>
...
const TraceExplorationHeader = ({
  controls,
  model,
}: TraceExplorationHeaderProps) => {
  const styles = useStyles2(getStyles);
  ...
  const dsVariable = sceneGraph.lookupVariable(
    VAR_DATASOURCE,
    traceExploration,
  );
  const filtersVariable = getFiltersVariable(traceExploration);
    ...
    <div className={styles.headerContainer}>
      <Stack gap={1} justifyContent={"space-between"} wrap={"wrap"}>
          ...
          {dsVariable && (
            <Stack gap={0} alignItems={"center"}>
              <div className={styles.datasourceLabel}>
                <Trans i18nKey="trace-exploration.header.data-source">
                  Data source
                </Trans>
              </div>
        ...
        <div className={styles.controls}>
          ...
          {traceExploration.state.loadSearchScene && (
            <traceExploration.state.loadSearchScene.Component
              model={traceExploration.state.loadSearchScene}
            />
          )}
      ...
      <Stack gap={1} alignItems={"flex-start"} justifyContent={"space-between"}>
        ...
        <Stack gap={0} alignItems={"center"}>
          ...
          <Input
            placeholder={t(
              "trace-exploration.header.trace-id-placeholder",
              ...
              <Stack direction="row" alignItems="center" gap={1} width="40px">
                {localTraceId && (
                  <>
                    <Icon
                      name="times"
                      onClick={() => setLocalTraceId("")}
                      cursor="pointer"
                    />
                    <Icon
                      name="enter"
                      onClick={onTraceIdSubmit}
                      cursor="pointer"
                    />
                  </>
                )}
...
function getTopScene(state: TraceExplorationState) {
  ...
  const isKnownView = actionViewsDefinitions.some(
  ...
  if (
    state.allowedActionViews?.length &&
    !state.allowedActionViews.includes(requestedView)
  ) {
    return new TracesByServiceScene({});
  }
  ...
  return new SceneVariableSet({
    variables: [
      ...
      new CustomVariable({
        ...
        label: t(
          "trace-exploration.variable.duration-percentiles",
